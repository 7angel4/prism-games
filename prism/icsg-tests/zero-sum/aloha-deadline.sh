...
run_experiments() {
  ...
  # Write CSV header
  echo "\"bmax,D\",Actions_max/avg,Val_iters,Qual_verif_time,Quant_verif_time,Value" >> "$RESULTS_FILE"
  ...
  extract_results() {
    ...
    while true; do
      ...
      QUANT_TIME=$(echo "$OUTPUT" | grep 'Time for model checking:' | head -1 | sed -E 's/.*Time for model checking: ([0-9\.]+).*/\1/')
      QUANT_TIME=$(echo "$QUANT_TIME $QUAL_TIME" | tr -d '[:space:]' | awk -F'-' '{print $1-$2}' | xargs printf "%.2f")
    ...
    echo "\"$BMAX,$D\",\"$MAX_AVG_ACTIONS\",$VAL_ITERS,$QUAL_TIME,$QUANT_TIME,$VALUE" >> "$RESULTS_FILE"
  }
