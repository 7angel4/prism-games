	...
	public DTMCSimple(MDPSimple<Value> mdp)
	{
		...
		for (int s = 0; s < numStates; s++) {
			...
			if (numChoices == 0) {
				...
				Map<Integer, Value> agg = new HashMap<>();
				for (int c = 0; c < numChoices; c++) {
					Distribution<Value> distr = mdp.getChoice(s, c);
					for (Map.Entry<Integer, Value> e : distr) {
						agg.merge(e.getKey(), e.getValue(), getEvaluator()::add);
					}
				}
				Value total = agg.values().stream().reduce(getEvaluator()::add).orElse(getEvaluator().zero());
				if (getEvaluator().isZero(total)) continue;
				for (Map.Entry<Integer, Value> e : agg.entrySet()) {
					addToProbability(s, e.getKey(), getEvaluator().divide(e.getValue(), total));
				}
			}
